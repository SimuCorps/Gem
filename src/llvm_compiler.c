...
static void createRuntimeLibrary(const char* runtimePath) {
    ...
    fprintf(file, "            // For strings, the data field contains a pointer to the string constant\n");
    fprintf(file, "            if (value.data != 0) {\n");
    fprintf(file, "                // The data field contains a pointer to the actual string data\n");
    ...
    fprintf(file, "    if (a.type == VAL_OBJ && b.type == VAL_OBJ) {\n");
    fprintf(file, "        // Both should be strings for concatenation\n");
    fprintf(file, "        char* aStr = (char*)a.data;\n");
    fprintf(file, "        char* bStr = (char*)b.data;\n");
    fprintf(file, "        \n");
    fprintf(file, "        if (aStr && bStr) {\n");
    fprintf(file, "            int aLen = strlen(aStr);\n");
    fprintf(file, "            int bLen = strlen(bStr);\n");
    fprintf(file, "            int totalLen = aLen + bLen;\n");
    fprintf(file, "            \n");
    fprintf(file, "            // Use static buffer for simplicity (not thread-safe but works for demo)\n");
    fprintf(file, "            static char resultBuffer[1024];\n");
    fprintf(file, "            if (totalLen < 1023) {\n");
    fprintf(file, "                memcpy(resultBuffer, aStr, aLen);\n");
    fprintf(file, "                memcpy(resultBuffer + aLen, bStr, bLen);\n");
    fprintf(file, "                resultBuffer[totalLen] = '\\0';\n");
