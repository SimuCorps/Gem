...
static LLVMValueRef allocateFunction;
...
void initLLVMCompiler() {
    ...
    // For now, we'll use a simple struct with type tag and union
...
static LLVMValueRef createConstantValue(Value value) {
    ...
    if (IS_BOOL(value)) {
        ...
        // For objects, we'll need more complex handling
        typeTag = LLVMConstInt(LLVMInt32TypeInContext(context), 3, 0); // OBJ type
        data = LLVMConstInt(LLVMInt64TypeInContext(context), 0, 0); // placeholder
    }
